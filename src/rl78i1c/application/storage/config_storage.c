...
***********************************************************************************************************************/
/* Variables */
static CONFIG_STORAGE_HEADER       g_config_storage_header;                               /* Storage Header */
static CONFIG_STATE                g_config_storage_state = CONFIG_STATE_UNINITIALIZED;   /* Storage State */

/* CRC interval checking calibration data in RAM */
static uint8_t                     g_config_storage_interval_flag;                        /* Storage interval checking CRC for RAM */
static uint8_t                     g_config_disable_CRC_checking;                         /* Enable CRC checking */
...
{	
    ...
    float32_t i_phase_degrees_phase_r_temp[EM_GAIN_PHASE_NUM_LEVEL_MAX];
    float32_t i_phase_degrees_phase_y_temp[EM_GAIN_PHASE_NUM_LEVEL_MAX];
    float32_t i_phase_degrees_phase_b_temp[EM_GAIN_PHASE_NUM_LEVEL_MAX];
    float32_t i_gain_values_phase_r_temp[EM_GAIN_PHASE_NUM_LEVEL_MAX];
    float32_t i_gain_values_phase_y_temp[EM_GAIN_PHASE_NUM_LEVEL_MAX];
    float32_t i_gain_values_phase_b_temp[EM_GAIN_PHASE_NUM_LEVEL_MAX];
	uint8_t i;
    /* Pass data to temporary for phase degree */
    for ( i = 0; i < 2; i++ )
    {
        i_phase_degrees_phase_r_temp[i] = calib.sw_phase_correction.phase_r.i_phase_degrees[i];
        i_phase_degrees_phase_y_temp[i] = calib.sw_phase_correction.phase_y.i_phase_degrees[i];
        i_phase_degrees_phase_b_temp[i] = calib.sw_phase_correction.phase_b.i_phase_degrees[i];

        i_gain_values_phase_r_temp[i]   = calib.sw_gain.phase_r.i_gain_values[i];
        i_gain_values_phase_y_temp[i]   = calib.sw_gain.phase_y.i_gain_values[i];
        i_gain_values_phase_b_temp[i]   = calib.sw_gain.phase_b.i_gain_values[i];
    }
    ...
    */
    ...
    if (DATAFLASH_Write(  CONFIG_STORAGE_CALIB_SW_DEGREE_ARRAY_PHASE_R_ADDR, 
                        (uint8_t *)&i_phase_degrees_phase_r_temp,
    ...
    if (DATAFLASH_Write(  CONFIG_STORAGE_CALIB_SW_DEGREE_ARRAY_PHASE_Y_ADDR, 
                        (uint8_t *)&i_phase_degrees_phase_y_temp,
    ...
    if (DATAFLASH_Write(  CONFIG_STORAGE_CALIB_SW_DEGREE_ARRAY_PHASE_B_ADDR, 
                        (uint8_t *)&i_phase_degrees_phase_b_temp,
    ...
    */
    ...
    if (    DATAFLASH_Write(  CONFIG_STORAGE_CALIB_SW_GAIN_ARRAY_PHASE_R_ADDR, 
                      (uint8_t *)&i_gain_values_phase_r_temp,
    ...
    if (    DATAFLASH_Write(  CONFIG_STORAGE_CALIB_SW_GAIN_ARRAY_PHASE_Y_ADDR, 
                      (uint8_t *)&i_gain_values_phase_y_temp,
    ...
    if (    DATAFLASH_Write(  CONFIG_STORAGE_CALIB_SW_GAIN_ARRAY_PHASE_B_ADDR, 
                      (uint8_t *)&i_gain_values_phase_b_temp,
...
CFS_FUNC uint8_t CONFIG_Format(void)
{
    ...
    /* Write default data */
    /* EM configuration default */
    /* Convert default gain value for phase r */
#if(EM_ADC_CURRENT_DRIVER_CHANNEL_OPTIMIZATION_PHASE_R == 0 )
    convert_default_gain_reg_value(EM_ADC_CURRENT_DRIVER_CHANNEL_PHASE_R, R_DSADC_GetGainEnumValue(EM_CALIB_DEFAULT_GAIN_PHASE_R), &regs.gain0, &regs.gain1);
#endif
    /* Convert default gain value for phase y*/
#if(EM_ADC_CURRENT_DRIVER_CHANNEL_OPTIMIZATION_PHASE_Y == 1 )
    convert_default_gain_reg_value(EM_ADC_CURRENT_DRIVER_CHANNEL_PHASE_Y, R_DSADC_GetGainEnumValue(EM_CALIB_DEFAULT_GAIN_PHASE_Y), &regs.gain0, &regs.gain1);
#endif
    /* Convert default gain value for phase b */
#if(EM_ADC_CURRENT_DRIVER_CHANNEL_OPTIMIZATION_PHASE_B == 2 )
    convert_default_gain_reg_value(EM_ADC_CURRENT_DRIVER_CHANNEL_PHASE_B, R_DSADC_GetGainEnumValue(EM_CALIB_DEFAULT_GAIN_PHASE_B), &regs.gain0, &regs.gain1);
#endif

    /* Convert default gain value for neutral */
#if(EM_ADC_CURRENT_DRIVER_CHANNEL_OPTIMIZATION_NEUTRAL == 3 )
    convert_default_gain_reg_value(EM_ADC_CURRENT_DRIVER_CHANNEL_NEUTRAL, R_DSADC_GetGainEnumValue(EM_CALIB_DEFAULT_GAIN_NEUTRAL), &regs.gain0, &regs.gain1);
#endif
...
CFS_FUNC uint8_t CONFIG_Restore(uint8_t selection)
{
    ...
    {        
        ...
        calib.sw_phase_correction.phase_r.i_phase_degrees = em_hold_setting_value.degree_list_r;
        calib.sw_phase_correction.phase_y.i_phase_degrees = em_hold_setting_value.degree_list_y;
        calib.sw_phase_correction.phase_b.i_phase_degrees = em_hold_setting_value.degree_list_b;

        /* Assign pointer for gain in calib to holder value */
        calib.sw_gain.phase_r.i_gain_values = em_hold_setting_value.gain_list_r;
        calib.sw_gain.phase_y.i_gain_values = em_hold_setting_value.gain_list_y;
        calib.sw_gain.phase_b.i_gain_values = em_hold_setting_value.gain_list_b;

        /* Get calibration from storage */
	if (CONFIG_LoadEMCalib(&calib, &em_hold_setting_value.regs) != CONFIG_OK)
        {
            return CONFIG_ERROR;
        }
...
{	
    ...
    for( i = 0; i < 2; i++)
    {
        p_calib->sw_phase_correction.phase_r.i_phase_degrees[i] = 0;
        p_calib->sw_phase_correction.phase_y.i_phase_degrees[i] = 0;
        p_calib->sw_phase_correction.phase_b.i_phase_degrees[i] = 0;
        p_calib->sw_gain.phase_r.i_gain_values[i] = 0;
        p_calib->sw_gain.phase_y.i_gain_values[i] = 0;
        p_calib->sw_gain.phase_b.i_gain_values[i] = 0;
    }
    ...
    */
    ...
    if ( DATAFLASH_Read( CONFIG_STORAGE_CALIB_SW_DEGREE_ARRAY_PHASE_R_ADDR, 
                        (uint8_t *)p_calib->sw_phase_correction.phase_r.i_phase_degrees,
                        CONFIG_STORAGE_CALIB_SW_DEGREE_ARRAY_PHASE_R_SIZE   ) != DATAFLASH_OK)
    {
        return CONFIG_ERROR;   /* Read error */
    }
    ...
    if ( DATAFLASH_Read( CONFIG_STORAGE_CALIB_SW_DEGREE_ARRAY_PHASE_Y_ADDR, 
                        (uint8_t *)p_calib->sw_phase_correction.phase_y.i_phase_degrees,
                        CONFIG_STORAGE_CALIB_SW_DEGREE_ARRAY_PHASE_Y_SIZE   ) != DATAFLASH_OK)
    {
        return CONFIG_ERROR;   /* Read error */
    }
    ...
    if ( DATAFLASH_Read( CONFIG_STORAGE_CALIB_SW_DEGREE_ARRAY_PHASE_B_ADDR, 
                        (uint8_t *)p_calib->sw_phase_correction.phase_b.i_phase_degrees,
                        CONFIG_STORAGE_CALIB_SW_DEGREE_ARRAY_PHASE_B_SIZE   ) != DATAFLASH_OK)
    {
        return CONFIG_ERROR;   /* Read error */
    }
    ...
    */
    ...
    if ( DATAFLASH_Read( CONFIG_STORAGE_CALIB_SW_GAIN_ARRAY_PHASE_R_ADDR, 
                        (uint8_t *)p_calib->sw_gain.phase_r.i_gain_values,
                        CONFIG_STORAGE_CALIB_SW_GAIN_ARRAY_PHASE_R_SIZE ) != DATAFLASH_OK)
    {
      return CONFIG_ERROR;   /* Read error */
    }
    ...
    if ( DATAFLASH_Read( CONFIG_STORAGE_CALIB_SW_GAIN_ARRAY_PHASE_Y_ADDR, 
                        (uint8_t *)p_calib->sw_gain.phase_y.i_gain_values,
                        CONFIG_STORAGE_CALIB_SW_GAIN_ARRAY_PHASE_Y_SIZE ) != DATAFLASH_OK)
    {
      return CONFIG_ERROR;   /* Read error */
    }
    ...
    if ( DATAFLASH_Read( CONFIG_STORAGE_CALIB_SW_GAIN_ARRAY_PHASE_B_ADDR, 
                        (uint8_t *)p_calib->sw_gain.phase_b.i_gain_values,
                        CONFIG_STORAGE_CALIB_SW_GAIN_ARRAY_PHASE_B_SIZE ) != DATAFLASH_OK)
    {
      return CONFIG_ERROR;   /* Read error */
    }
