...
#define DC_high()	 {P1_bit.no4 = 1U;}
#define DC_low()	 {P1_bit.no4 = 0U;}
#define Reset_high() {P1_bit.no3 = 1U;}
#define Reset_low()  {P1_bit.no3 = 0U;}
#define CS_high()    {P1_bit.no2 = 1U;}
#define CS_low()     {P1_bit.no2 = 0U;}

/** @brief tx_buf must satisfy uint8_t const * const & tx_num must satisfy uint16_t*/
#define Spi_write_block(tx_buf, tx_num)	{R_CSI00_Send_app(tx_buf, tx_num);}

/** @brief Implements milli-second resolution delay*/
#define Delay_ms(ms) {R_TAU0_Channel4_ms_delay(ms);}
